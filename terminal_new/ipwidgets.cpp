...
IpWidgets::IpWidgets(QWidget *parent /* = 0 */)
{
    m_Separator = ".";
...
//        m_SpinBox[i]->setFixedWidth(40);
    ...
    setLayout(layout);
    m_SpinBox[0]->setValue(127);
    m_SpinBox[3]->setValue(1);
}
...
QString IpWidgets::getIpaddress()
{
    QString ipAddress;
    for(int i=0; i<3; i++)
    {
...
bool IpWidgets::eventFilter(QObject *object, QEvent *event)
{
    //如果object不是四个输入框，则返回
    int nowBox = -1;
    for(int i=0; i<4; i++)
