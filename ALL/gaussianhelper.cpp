...
GaussianHelper::GaussianHelper(QObject *parent)
	: QObject(parent)
{
	m_unitByte = 1;
 ...
 */
void GaussianHelper::train_from_file(const QVector<QString> &filenames, const QVector<int> &label)
{
 ...
 */
void GaussianHelper::train_from_bytes(const QByteArray &stream, int label)
{
	if (stream.size() == 0)
	{
		emit alreadyTrained(m_featMean, m_featStd, m_gate);
...
void GaussianHelper::update_paramsters(const Tensor &featMean,
...
void GaussianHelper::deal_label(const QVector<int> &label, std::map< int, std::vector<int> > &arg_label)
{
...
void GaussianHelper::group_by_label(const std::map< int, std::vector<int> > &arg_label,
...
void GaussianHelper::onAudioFormatInit(const QString &rate, const QString &chns, const QString &size)
{
