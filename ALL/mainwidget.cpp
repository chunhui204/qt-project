    ...
    connect(tcpServer, &QTcpServer::newConnection,
            ...
            {//获取通信socket，并显示ip
                ...
                connect(tcpSocket, &QTcpSocket::disconnected,
                        [=]()
                        {
                            labelConnection->setText(QString::fromLocal8Bit("连接断开"));
                        });
            });
...
{
    qcout << "----used "<< AudioBufUsed.available() << "---------free " << AudioBufFree.available();
//    emit spectrumTimeout();
}
...
void MainWidget::designMenu(void)
{
    ...
    QMenu *menufunction = m_menuBar->addMenu(QString::fromLocal8Bit("功能"));
    QMenu *menuDevice = m_menuBar->addMenu(QString::fromLocal8Bit("设备"));

    QAction *actionOpen = menuFile->addAction(QString::fromLocal8Bit("打开文件"));
    QAction *actionPath = menuFile->addAction(QString::fromLocal8Bit("保存路径"));
...
//        cout << "name " <<name;
            ...
            [=]()
    {
        ...
        QString dir = QFileDialog::getExistingDirectory(this, QString::fromLocal8Bit("请设置数据保存目录"), QDir::currentPath(),
                                                        QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
        emit savePathChanged(dir);
    });

    QAction *actionAudioFunc = menufunction->addAction(QString::fromLocal8Bit("音频"));
    QAction *actionVideoFunc = menufunction->addAction(QString::fromLocal8Bit("视频"));
    ...
    menuDevice->addAction(QString::fromLocal8Bit("设备"));
...
void MainWidget::connectUI()
{
    ...
    connect(audioWidget, &AudioWidget::localAudioInfo, this, &MainWidget::parseAudioInfo);
    //当对话框设置改变时通知终端进行重新配置
    qRegisterMetaType<AudioSettingFormat>("AudioSettingFormat");
    connect(setting, &Setting::audioFormatChanged, this, &MainWidget::onAudioFormatChanged);
    connect(setting, &Setting::audioFormatChanged, audioWidget, &AudioWidget::onAudioFormatChanged);
    ...
    connect(aplot_thread, &AudioPlotThread::labelPredicted, audioWidget, &AudioWidget::onLabelPredicted);
    qRegisterMetaType<QVector<int>>("QVector<int>");
    qRegisterMetaType<QVector<QString>>("QVector<QString>");
    connect(audioWidget, &AudioWidget::trainModel, aplot_thread, &AudioPlotThread::onTrainModel);
    connect(aplot_thread, &AudioPlotThread::alreadyTrained, audioWidget, &AudioWidget::onAlreadyTrained);
    // 更新保存路径后通知需要保存文件的类
    ...
    //timeout
    //connect(this, &MainWidget::spectrumTimeout, asptrm_thread, &SpectrumThread::onTimeout);
}
...
//通知终端重置麦克风配置
void MainWidget::onAudioFormatChanged(AudioSettingFormat format)
{
    if(tcpSocket != NULL)
    {
        QByteArray array;
        QDataStream stream(&array, QIODevice::WriteOnly);
        stream << QString("resetAudioFormat") << format.deviceName << format.sampleRates
               << format.channel << format.sampleSizes << format.codec;
        tcpSocket->write(array);
    }
}
...
void MainWidget::dealResponseFromClient()
{
    ...
    while(head == "audioDevice" && stream.atEnd()==false)//(stream.atEnd() == false)
    {
...
void MainWidget::parseAudioInfo(QByteArray response)
{
    AudioSettingFormat used_format;
