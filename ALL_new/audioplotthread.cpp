...
//    timeCount = QTime::currentTime();
    ...
    {
        int label = 0;//m_spectrum->predict_of_frame(data, audioFormat.sampleSize(), unitByte);
...
void AudioPlotThread::onAudioFormatInit(std::shared_ptr<DeviceInfo> devs)
{
...
void AudioPlotThread::onSavePathChanged(const QString &dir)
{
    saveDir = dir;
}
